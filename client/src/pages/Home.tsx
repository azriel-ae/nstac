      ...
      <main className="main-content">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            <div className="status-chip"><span className="status-dot" />Browser-only</div>
...
type AppMode = "api" | "api-menu" | "upl-menu" | "receipt-menu" | "fore" | "nsc" | "receipt" | "receipt-compare" | "upl" | "pointcoffe" | "astana" | VendorKind;

function ApiParserSelector({ onSelect, onBack }: { onSelect: (mode: AppMode) => void; onBack: () => void }) {
  const parsers: Array<[AppMode, string, string, string]> = [["astana", "CEK ASTANA", "Cek Data ASTANA", "JSON revenue · Gross, Service, Tax, Nett"], ["fore", "CEK FORE", "Cek Data FORE", "Transaksi counter, billing, total, dan pajak"], ["nsc", "CEK NSC", "Cek Data NSC", "Transaksi film, studio, order, dan kursi"], ["rotio", "CEK ROTIO", "Cek Data ROTIO", "Parser dan rumus pajak khusus ROTIO"], ["kai", "CEK KAI", "Cek Data KAI", "Parser dan rumus pajak khusus KAI"], ["hokben", "CEK HOKBEN", "Cek Data HOKBEN", "Parser dan rumus pajak khusus HOKBEN"], ["kopken", "CEK KOPKEN", "Cek Data KOPKEN", "Parser dan rumus pajak khusus KOPKEN"], ["fave", "CEK FAVE", "Cek Data FAVE", "Parser dan rumus pajak khusus FAVE"], ["sams", "CEK SAMS", "Cek Data SAMS", "Parser dan rumus pajak khusus SAMS"]];
  return <div className="mode-shell"><div className="mode-brand"><div className="brand-mark"><span>α</span></div><div><div className="brand-name">nstax</div><div className="brand-caption">transaction intelligence</div></div></div><div className="mode-content"><button className="button button-secondary compact" type="button" onClick={onBack}>← Kembali ke menu utama</button><div className="eyebrow"><span className="eyebrow-line" /> NSTAX · CEK DATA API</div><h1>Pilih parser<br /><em>data API.</em></h1><p>Gunakan parser yang sudah tersedia untuk memeriksa data sesuai format sumbernya.</p><div className="mode-cards">{parsers.map(([mode, label, title, detail]) => <button className="mode-card mode-card-small" type="button" key={mode} onClick={() => onSelect(mode)}><span className="mode-card-label">{label}</span><strong>{title}</strong><span>{detail}</span></button>)}</div><div className="mode-local"><ShieldCheck size={16} /> Semua data diproses lokal di browser Anda</div></div></div>;
}

function ApiSelector({ onSelect }: { onSelect: (mode: AppMode) => void }) {
  return <div className="mode-shell"><div className="mode-brand"><div className="brand-mark"><span>α</span></div><div><div className="brand-name">nstax</div><div className="brand-caption">transaction intelligence</div></div></div><div className="mode-content"><div className="eyebrow"><span className="eyebrow-line" /> NSTAX · MENU UTAMA</div><h1>Pilih jenis<br /><em>pemeriksaan.</em></h1><p>Kelola dan periksa data transaksi dengan tools yang sesuai untuk setiap kebutuhan.</p><div className="mode-cards main-menu-cards"><button className="mode-card" type="button" onClick={() => onSelect("api-menu")}><span className="mode-card-label">CEK DATA API</span><strong>Cek Data API</strong><span>ASTANA, FORE, NSC, ROTIO, KAI, HOKBEN, KOPKEN, FAVE, SAMS</span></button><button className="mode-card" type="button" onClick={() => onSelect("receipt-menu")}><span className="mode-card-label">CEK STRUK</span><strong>Baca Data Struk</strong><span>Semua fungsi CEK STRUK lama</span></button><button className="mode-card" type="button" onClick={() => onSelect("upl-menu")}><span className="mode-card-label">CEK UPL</span><strong>Periksa Upload File</strong><span>CEK KLAND dan CEK POINTCOFFE</span></button></div><div className="mode-local"><ShieldCheck size={16} /> Semua data diproses lokal di browser Anda</div></div></div>;
}

function UplModeSelector({ onSelect, onBack }: { onSelect: (mode: "upl" | "pointcoffe") => void; onBack: () => void }) {
  return <div className="mode-shell"><div className="mode-brand"><div className="brand-mark"><span>α</span></div><div><div className="brand-name">nstax</div><div className="brand-caption">transaction intelligence</div></div></div><div className="mode-content"><button className="button button-secondary compact" type="button" onClick={onBack}>← Kembali ke CEK DATA API</button><div className="eyebrow"><span className="eyebrow-line" /> CEK UPL</div><h1>Pilih sumber<br /><em>data UPL.</em></h1><div className="mode-cards"><button className="mode-card" type="button" onClick={() => onSelect("upl")}><span className="mode-card-label">CEK KLAND</span><strong>Periksa Upload Excel</strong><span>Parser Excel KLAND yang sudah ada</span></button><button className="mode-card" type="button" onClick={() => onSelect("pointcoffe")}><span className="mode-card-label">CEK POINTCOFFE</span><strong>Periksa CSV PointCoffee</strong><span>Delimiter pipe (|), DPP, Pajak, Total</span></button></div></div></div>;
}

function ReceiptModeSelector({ onSelect, onBack }: { onSelect: (mode: "receipt" | "receipt-compare") => void; onBack: () => void }) {
  return <div className="mode-shell"><div className="mode-brand"><div className="brand-mark"><span>α</span></div><div><div className="brand-name">nstax</div><div className="brand-caption">transaction intelligence</div></div></div><div className="mode-content"><button className="button button-secondary compact" type="button" onClick={onBack}>← Kembali ke menu utama</button><div className="eyebrow"><span className="eyebrow-line" /> CEK STRUK</div><h1>Pilih fitur<br /><em>struk.</em></h1><div className="mode-cards"><button className="mode-card" type="button" onClick={() => onSelect("receipt")}><span className="mode-card-label">CEK STRUK</span><strong>Periksa satu file struk</strong><span>Parser PDF, Excel, dan CSV lama</span></button><button className="mode-card" type="button" onClick={() => onSelect("receipt-compare")}><span className="mode-card-label">BANDINGKAN 2 FILE STRUK</span><strong>Compare dua file struk</strong><span>Bandingkan berdasarkan no_struk dan field transaksi</span></button></div></div></div>;
}
